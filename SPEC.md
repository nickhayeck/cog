...
# Cog Language Specification (Draft)
...
- No safety guarantees for pointer dereference; Cog does not attempt to make raw pointer dereference memory-safe.
...
`fn struct enum trait impl for type const static mod use pub as let mut if else while loop match return break continue comptime dyn Self true false`
...
- Slices: `[T]` is an unsized slice type; it appears behind pointers. `const* [T]` / `mut* [T]` are fat pointers `{ data_ptr, len: usize }`.
...
- “Scoped pointers” may exist later; conversions between scoped and raw are restricted (details TBD).

### 4.4 Trait objects
- `const* dyn Trait` / `mut* dyn Trait` are trait-object fat pointers:
  - data pointer
  - vtable pointer
- Calling a method on `dyn Trait` uses vtable dispatch.

ABI (initial):
- Layout is `{ data_ptr, vtable_ptr }`.
- The vtable is ordered by **trait declaration order**.
- Vtable entries are function pointers with an erased `self` (`i8*`-like) plus the method’s other parameters.
- Methods that are not dyn-callable may have a null vtable entry; attempting a dyn call to such a method is a compile-time error.
...
- Eventually, functions will be executable by the comptime interpreter when called from comptime contexts (subject to restrictions and a step budget). The surface syntax for “comptime-callable” functions is TBD.
- Comptime execution is deterministic and side-effect constrained (I/O, syscalls, threads are disallowed at comptime).
...
The prototype uses the namespace `builtin::...`.

Implemented (v0.0.x):
- `builtin::size_of(T)` / `builtin::align_of(T)`:
  - comptime-only; `T` is a type value (currently a type path like `i32` or `Foo`)
  - returns `usize`
- `builtin::compile_error("message")`:
  - comptime-only; reports an error when executed during comptime evaluation
  - treated as a diverging expression (it “does not produce a value”)
- `builtin::addr_of(place)` / `builtin::addr_of_mut(place)`:
  - produces `const* T` / `mut* T` for a place expression

Planned:
- `builtin::type_info(T)` for reflection.
- `builtin::type_of(x)` once Cog has a first-class meta-typing story.
