...
class LayoutEngine {
 public:
  ...
  std::optional<StructLayout> struct_layout(const ItemStruct* def, Span use_site);
  std::optional<EnumLayout> enum_layout(const ItemEnum* def, Span use_site);
  std::optional<std::uint64_t> struct_field_offset(const ItemStruct* def, std::string_view field, Span use_site);
 ...
 private:
  ...
  std::optional<StructLayout> compute_struct_layout(const ItemStruct* def, Span use_site);
  std::optional<EnumLayout> compute_enum_layout(const ItemEnum* def, Span use_site);
