...
static std::optional<TargetLayout> compute_layout_from_datalayout(const llvm::DataLayout& dl) {
  TargetLayout out{};
  ...
  out.i128_align = dl.getABITypeAlign(llvm::IntegerType::get(ctx, 128)).value();
  return out;
}

}  // namespace

std::optional<TargetSpec> compute_target_spec(Session& session, std::optional<std::string_view> triple_arg) {
  ...
  std::string triple = triple_arg ? std::string(*triple_arg) : llvm::sys::getProcessTriple();
  ...
  if (!target) {
    session.diags.push_back(Diagnostic{
        ...
        .span = Span{},
        .message = "LLVM target lookup failed for `" + triple + "`: " + error,
    });
    return std::nullopt;
  }
  ...
  auto tm = std::unique_ptr<llvm::TargetMachine>(target->createTargetMachine(tt, cpu, features, opt, reloc));
  if (!tm) {
    session.diags.push_back(Diagnostic{
        .severity = Severity::Error,
        .span = Span{},
        .message = "failed to create LLVM TargetMachine for `" + triple + "`",
    });
    return std::nullopt;
  }
