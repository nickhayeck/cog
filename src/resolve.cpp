...
class Resolver {
 public:
  ...
  ResolvedCrate run(FileId root_file) {
    ...
    if (!root_parsed || !root_parsed->root) return crate_;

    std::filesystem::path root_path = std::filesystem::path(session_.sources.path(root_file));
    std::filesystem::path root_dir = root_path.parent_path();

    crate_.root = add_module("<crate>", kNoParent, root_file, root_dir, root_parsed->root->items);
    load_submodules(crate_.root);

    for (ModuleId id = 0; id < crate_.modules.size(); id++) collect_defs(id);
    resolve_all_uses();
    collect_all_impls();
 ...
 private:
  ...
  std::optional<std::filesystem::path> find_mod_file(const Module& parent, std::string_view name, Span span) {
    ...
    if (a_exists && b_exists) {
      error(span, "ambiguous module file for `mod " + std::string(name) + ";` (found both `" + a.string() +
                       "` and `" + b.string() + "`)");
      return std::nullopt;
    }
    ...
    error(span, "could not find module file for `mod " + std::string(name) + ";` (tried `" + a.string() + "` and `" +
                     b.string() + "`)");
    return std::nullopt;
  }
    ...
    for (Item* item : parent_items) {
      ...
      switch (item->kind) {
        case AstNodeKind::ItemModInline: {
          ...
          if (crate_.modules[parent_id].submodules.contains(mod->name)) {
            error(mod->span, "duplicate module `" + mod->name + "`");
            break;
          }
          std::filesystem::path child_dir = parent_dir / mod->name;
          ModuleId child_id = add_module(mod->name, parent_id, mod->span.file, child_dir, mod->items);
          crate_.modules[parent_id].submodules.insert({mod->name, child_id});
        ...
        case AstNodeKind::ItemModDecl: {
          ...
          auto file_path_opt = find_mod_file(crate_.modules[parent_id], mod->name, mod->span);
          ...
          std::filesystem::path child_dir = parent_dir / mod->name;
          ModuleId child_id = add_module(mod->name, parent_id, file, child_dir, parsed->root->items);
          crate_.modules[parent_id].submodules.insert({mod->name, child_id});
          load_submodules(child_id);
          break;
        }
    ...
    for (Item* item : m.items) {
      ...
      if (is_type_item(item)) {
        ...
        if (m.types.contains(name)) {
          error(item->span, "duplicate type name `" + name + "` in module `" + m.name + "`");
          continue;
        }
      ...
      if (is_value_item(item)) {
        ...
        if (m.values.contains(name)) {
          error(item->span, "duplicate value name `" + name + "` in module `" + m.name + "`");
          continue;
        }
  ...
  std::optional<ModuleId> resolve_module_in(ModuleId module_id, std::string_view name) {
  ...
  Item* resolve_type_in(ModuleId module_id, std::string_view name) {
    const Module& m = crate_.modules[module_id];
    if (auto it = m.types.find(std::string(name)); it != m.types.end()) return it->second;
    return nullptr;
  }

  Item* resolve_value_in(ModuleId module_id, std::string_view name) {
    const Module& m = crate_.modules[module_id];
    if (auto it = m.values.find(std::string(name)); it != m.values.end()) return it->second;
    return nullptr;
  }

  std::optional<ModuleId> resolve_module_path(ModuleId from, const Path* path) {
    ...
    for (const Ident* seg : path->segments) {
      ...
      if (!next) {
        error(seg->span, "cannot find module `" + std::string(name) + "` in module `" + crate_.modules[cur].name + "`");
        return std::nullopt;
      }
      cur = *next;
    }
  ...
  std::optional<Resolved> resolve_path_any(ModuleId from, const Path* path) {
    ...
    for (size_t i = 0; i + 1 < path->segments.size(); i++) {
      ...
      if (!next) {
        error(seg->span, "cannot find module `" + seg->text + "` in module `" + crate_.modules[cur].name + "`");
        return std::nullopt;
      }
      cur = *next;
    }
    ...
    if (auto mod = resolve_module_in(cur, name)) return Resolved{.kind = ResolvedKind::Module, .module = *mod};
    if (Item* ty = resolve_type_in(cur, name)) return Resolved{.kind = ResolvedKind::Type, .item = ty};
    if (Item* val = resolve_value_in(cur, name)) return Resolved{.kind = ResolvedKind::Value, .item = val};

    error(last->span,
          "cannot find item `" + std::string(name) + "` in module `" + crate_.modules[cur].name + "`");
    return std::nullopt;
  }

  bool add_import(ModuleId into, std::string name, const Resolved& r, Span span) {
    Module& m = crate_.modules[into];
    switch (r.kind) {
      case ResolvedKind::Module:
        if (auto it = m.submodules.find(name); it != m.submodules.end()) {
          if (it->second == r.module) return true;
          error(span, "duplicate module binding `" + name + "` in module `" + m.name + "`");
          return false;
        }
      ...
      case ResolvedKind::Type:
        if (auto it = m.types.find(name); it != m.types.end()) {
          ...
          error(span, "duplicate type binding `" + name + "` in module `" + m.name + "`");
          return false;
        }
      ...
      case ResolvedKind::Value:
        if (auto it = m.values.find(name); it != m.values.end()) {
          if (it->second == r.item) return true;
          error(span, "duplicate value binding `" + name + "` in module `" + m.name + "`");
          return false;
        }
  ...
  void resolve_use_tree(ModuleId into, ModuleId base, const UseTree* tree) {
    ...
    if (!tree->group.empty()) {
      ...
      if (!mod_id) return;
      for (const UseTree* child : tree->group) resolve_use_tree(into, *mod_id, child);
      return;
    }
    ...
    std::string import_name = tree->alias.empty() ? tree->path->segments.back()->text : tree->alias;
  ...
  Item* resolve_type_path(ModuleId from, const Path* path) {
    ...
    if (!ty) {
      error(last->span, "cannot find type `" + last->text + "` in module `" + crate_.modules[cur].name + "`");
      return nullptr;
    }
    return ty;
  }
