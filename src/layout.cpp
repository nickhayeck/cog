...
std::optional<std::uint64_t> LayoutEngine::array_len_value(const Expr* expr) const {
  ...
  if (auto it = array_lens_.find(expr); it != array_lens_.end()) return it->second;
  return std::nullopt;
}
...
std::optional<StructLayout> LayoutEngine::struct_layout(const ItemStruct* def, Span use_site) {
  if (!def) return std::nullopt;
  if (auto it = struct_cache_.find(def); it != struct_cache_.end()) return it->second;
...
std::optional<EnumLayout> LayoutEngine::enum_layout(const ItemEnum* def, Span use_site) {
  if (!def) return std::nullopt;
  if (auto it = enum_cache_.find(def); it != enum_cache_.end()) return it->second;
...
std::optional<std::uint64_t> LayoutEngine::struct_field_offset(const ItemStruct* def, std::string_view field, Span use_site) {
...
std::optional<Layout> LayoutEngine::compute_layout(TypeId ty, Span use_site) {
  ...
  switch (d.kind) {
    ...
    case TypeKind::Array: {
      ...
      if (!len) {
        error(use_site, "array length is not a known comptime constant");
        return std::nullopt;
      }
    ...
    case TypeKind::Struct: {
      if (!d.struct_def) return Layout{.size = 0, .align = 1, .sized = true};
      auto sl = struct_layout(d.struct_def, use_site);
      if (!sl) return std::nullopt;
      return sl->layout;
    }
    case TypeKind::Enum: {
      if (!d.enum_def) return Layout{.size = 0, .align = 1, .sized = true};
...
std::optional<StructLayout> LayoutEngine::compute_struct_layout(const ItemStruct* def, Span use_site) {
  ...
  for (const auto& f : it->second.fields_in_order) {
    ...
    offset = align_up(offset, fa);
    out.fields.push_back(FieldLayout{.name = f.name, .type = f.type, .offset = offset});
    offset += fl->size;
    max_align = std::max(max_align, fa);
  }
...
std::optional<EnumLayout> LayoutEngine::compute_enum_layout(const ItemEnum* def, Span use_site) {
  ...
  if (n <= 0x100) tag_size = 1;
  else if (n <= 0x10000) tag_size = 2;
