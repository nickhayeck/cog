...
struct Attr final : AstNode {
    ...
    explicit Attr(Span span, Path* name)
        : AstNode(AstNodeKind::Attr, span), name(name) {}
    ...
    explicit Attr(Span span, Path* name, std::string arg_string)
        : AstNode(AstNodeKind::Attr, span),
          name(name),
          arg_string(std::move(arg_string)) {}
};
...
struct TypeSlice final : Type {
    Type* elem = nullptr;
    explicit TypeSlice(Span span, Type* elem)
        : Type(AstNodeKind::TypeSlice, span), elem(elem) {}
};
...
struct TypeFn final : Type {
    std::vector<Type*> params{};
    Type* ret = nullptr;  // optional; null means ()
    explicit TypeFn(Span span, std::vector<Type*> params, Type* ret)
        : Type(AstNodeKind::TypeFn, span),
...
struct Block final : AstNode {
    ...
    explicit Block(Span span, std::vector<Stmt*> stmts, Expr* tail)
        : AstNode(AstNodeKind::Block, span),
...
struct MatchArm final : AstNode {
    ...
    explicit MatchArm(Span span, Pattern* pat, Expr* guard, Expr* body)
        : AstNode(AstNodeKind::MatchArm, span),
          pat(pat),
          guard(guard),
          body(body) {}
};
...
struct FieldInit final : AstNode {
    ...
    explicit FieldInit(Span span, std::string name, Expr* value)
        : AstNode(AstNodeKind::FieldInit, span),
          name(std::move(name)),
          value(value) {}
};
...
struct PatInt final : Pattern {
    std::int64_t value = 0;
    explicit PatInt(Span span, std::int64_t value)
        : Pattern(AstNodeKind::PatInt, span), value(value) {}
};

struct PatBool final : Pattern {
    bool value = false;
    explicit PatBool(Span span, bool value)
        : Pattern(AstNodeKind::PatBool, span), value(value) {}
};
...
struct PatBinding final : Pattern {
    ...
    explicit PatBinding(Span span, bool is_mut, std::string name)
        : Pattern(AstNodeKind::PatBinding, span),
          is_mut(is_mut),
          name(std::move(name)) {}
};
...
struct PatField final : AstNode {
    ...
    explicit PatField(Span span, std::string name, Pattern* pat)
        : AstNode(AstNodeKind::PatField, span),
          name(std::move(name)),
          pat(pat) {}
};
...
struct PatStruct final : Pattern {
    ...
    explicit PatStruct(Span span, Path* type_name,
                       std::vector<PatField*> fields)
        : Pattern(AstNodeKind::PatStruct, span),
          type_name(type_name),
...
struct PatVariant final : Pattern {
    ...
    explicit PatVariant(Span span, Path* path, std::vector<Pattern*> args)
        : Pattern(AstNodeKind::PatVariant, span),
          path(path),
          args(std::move(args)) {}
};
...
struct PatPath final : Pattern {
    ...
    explicit PatPath(Span span, Path* path)
        : Pattern(AstNodeKind::PatPath, span), path(path) {}
};
...
struct ExprInt final : Expr {
    std::int64_t value = 0;
    explicit ExprInt(Span span, std::int64_t value)
...
struct ExprBool final : Expr {
    ...
    explicit ExprBool(Span span, bool value)
        : Expr(AstNodeKind::ExprBool, span), value(value) {}
};
...
struct ExprString final : Expr {
    ...
    explicit ExprString(Span span, std::string value, bool is_c_string = false)
        : Expr(AstNodeKind::ExprString, span),
...
struct ExprBlock final : Expr {
    Block* block = nullptr;
    explicit ExprBlock(Span span, Block* block)
        : Expr(AstNodeKind::ExprBlock, span), block(block) {}
};

struct ExprComptime final : Expr {
    Block* block = nullptr;
    explicit ExprComptime(Span span, Block* block)
        : Expr(AstNodeKind::ExprComptime, span), block(block) {}
};
...
struct ExprLoop final : Expr {
    Block* body = nullptr;
    explicit ExprLoop(Span span, Block* body)
        : Expr(AstNodeKind::ExprLoop, span), body(body) {}
};
...
struct ExprMatch final : Expr {
    ...
    explicit ExprMatch(Span span, Expr* scrutinee, std::vector<MatchArm*> arms)
        : Expr(AstNodeKind::ExprMatch, span),
          scrutinee(scrutinee),
          arms(std::move(arms)) {}
};
...
struct ExprCall final : Expr {
    ...
    explicit ExprCall(Span span, Expr* callee, std::vector<Expr*> args)
        : Expr(AstNodeKind::ExprCall, span),
          callee(callee),
          args(std::move(args)) {}
};
...
struct ExprMethodCall final : Expr {
    ...
    std::string method{};
    std::vector<Expr*> args{};
    explicit ExprMethodCall(Span span, Expr* receiver, std::string method,
                            std::vector<Expr*> args)
...
struct ExprField final : Expr {
    ...
    explicit ExprField(Span span, Expr* base, std::string field)
        : Expr(AstNodeKind::ExprField, span),
          base(base),
...
struct ExprStructLit final : Expr {
    ...
    explicit ExprStructLit(Span span, Path* type_name,
                           std::vector<FieldInit*> inits)
        : Expr(AstNodeKind::ExprStructLit, span),
          type_name(type_name),
...
struct StmtExpr final : Stmt {
    Expr* expr = nullptr;
    explicit StmtExpr(Span span, Expr* expr)
        : Stmt(AstNodeKind::StmtExpr, span), expr(expr) {}
};

struct StmtReturn final : Stmt {
    Expr* value = nullptr;  // optional
    explicit StmtReturn(Span span, Expr* value)
        : Stmt(AstNodeKind::StmtReturn, span), value(value) {}
};

struct StmtBreak final : Stmt {
    Expr* value = nullptr;  // optional
    explicit StmtBreak(Span span, Expr* value)
        : Stmt(AstNodeKind::StmtBreak, span), value(value) {}
};
...
struct Param final : AstNode {
    ...
    explicit Param(Span span, bool is_comptime, std::string name, Type* type)
        : AstNode(AstNodeKind::Param, span),
...
struct FnSig final : AstNode {
    ...
    explicit FnSig(Span span, std::vector<Param*> params, Type* ret,
                   bool is_variadic = false)
        : AstNode(AstNodeKind::FnSig, span),
          params(std::move(params)),
          ret(ret),
...
struct FieldDecl final : AstNode {
    ...
    std::string name{};
    Type* type = nullptr;
    explicit FieldDecl(Span span, std::vector<Attr*> attrs, Visibility vis,
...
struct VariantDecl final : AstNode {
    ...
    explicit VariantDecl(Span span, std::string name,
                         std::vector<Type*> payload,
...
struct UseTree final : AstNode {
    ...
    explicit UseTree(Span span, Path* path, std::string alias,
                     std::vector<UseTree*> group)
...
struct Item : AstNode {
    std::vector<Attr*> attrs{};
    Visibility vis = Visibility::Private;
    explicit Item(AstNodeKind kind, Span span, std::vector<Attr*> attrs,
                  Visibility vis)
...
struct ItemUse final : Item {
    ...
    explicit ItemUse(Span span, std::vector<Attr*> attrs, Visibility vis,
                     UseTree* tree)
...
struct ItemModInline final : Item {
    ...
    explicit ItemModInline(Span span, std::vector<Attr*> attrs, Visibility vis,
                           std::string name, std::vector<Item*> items)
...
struct ItemModDecl final : Item {
    std::string name{};
    explicit ItemModDecl(Span span, std::vector<Attr*> attrs, Visibility vis,
                         std::string name)
        : Item(AstNodeKind::ItemModDecl, span, std::move(attrs), vis),
          name(std::move(name)) {}
};
...
struct ItemStruct final : Item {
    ...
    explicit ItemStruct(Span span, std::vector<Attr*> attrs, Visibility vis,
                        std::string name, std::vector<FieldDecl*> fields)
...
struct ItemEnum final : Item {
    ...
    explicit ItemEnum(Span span, std::vector<Attr*> attrs, Visibility vis,
                      std::string name, std::vector<VariantDecl*> variants)
...
struct ItemFn final : Item {
    FnDecl* decl = nullptr;
    Block* body = nullptr;
    explicit ItemFn(Span span, std::vector<Attr*> attrs, Visibility vis,
                    FnDecl* decl, Block* body)
        : Item(AstNodeKind::ItemFn, span, std::move(attrs), vis),
          decl(decl),
          body(body) {}
};
...
struct ItemConst final : Item {
    ...
    Type* type = nullptr;
    Expr* value = nullptr;
    explicit ItemConst(Span span, std::vector<Attr*> attrs, Visibility vis,
                       std::string name, Type* type, Expr* value)
...
struct ItemStatic final : Item {
    ...
    Type* type = nullptr;
    Expr* value = nullptr;
    explicit ItemStatic(Span span, std::vector<Attr*> attrs, Visibility vis,
                        std::string name, Type* type, Expr* value)
...
struct ItemTypeAlias final : Item {
    ...
    explicit ItemTypeAlias(Span span, std::vector<Attr*> attrs, Visibility vis,
                           std::string name, Type* aliased)
...
struct ItemImplInherent final : Item {
    ...
    explicit ItemImplInherent(Span span, std::vector<Attr*> attrs,
                              Visibility vis, Path* type_name,
                              std::vector<ItemFn*> methods)
